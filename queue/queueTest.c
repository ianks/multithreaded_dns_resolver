...
int main(int argc, char* argv[]) {
  ...
  /* Setup payload_in as int* array from 0 to TEST_SIZE-1 */
  ...
  if(!queue_is_empty(&q)) {
    ...
    fprintf(stderr, "queue_is_empty reports that the queue is not empty\n");
  }
  ...
  if(queue_is_empty(&q)) {
    fprintf(stderr, "error: queue should report full\n");
    fprintf(stderr, "queue_is_empty reports that the queue is empty\n");
  }

  /* Test for full queue when full */
  if(!queue_is_full(&q)) {
    fprintf(stderr, "error: queue should report full\n");
    fprintf(stderr, "queue_is_full reports that the queue is not full\n");
  }

  /* Test that push fails when full */
  if(!queue_push(&q, payload_in[0]) == QUEUE_FAILURE) {
    fprintf(stderr, "error: queue_push did not fail when full!\n");
  }
  ...
  for(i=0; i<TEST_SIZE; i++) {
    if(payload_in[i] != payload_out[i]) {
      fprintf(stderr, "error: push/pop mismatch!\n"
              "Payload Index: %d, "
              "Input Value: %d, "
              "Output Value: %d\n", i, *(payload_in[i]), *(payload_out[i]));
    }
  }
  ...
  if(queue_pop(&q)) {
    fprintf(stderr, "error: queue_pop did not return NULL when empty!\n");
  }
